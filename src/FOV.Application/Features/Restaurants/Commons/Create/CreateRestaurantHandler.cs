...
public sealed record CreateRestaurantCommand : IRequest<CreateRestaurantResponse>
{
...
public record CreateRestaurantResponse(Guid Id, string ManagerCode, string HeadchefCode);
public partial class CreateRestaurantHandler(IUnitOfWorks unitOfWorks, UserManager<User> userManager, RoleManager<ApplicationRole> roleManager) : IRequestHandler<CreateRestaurantCommand, CreateRestaurantResponse>
{
    ...
    public async Task<CreateRestaurantResponse> Handle(CreateRestaurantCommand request, CancellationToken cancellationToken)
    {
        ...
        var generateManagerCode = await GenerateCode(1);
        var generateHeadchefCode = await GenerateCode(4);
        ...
        var managerResult = await _userManager.CreateAsync(manager, "12345678!Fpt");
        if (!managerResult.Succeeded)
        {
            throw new AppException("Không thể tạo Manager");
        }
        await _userManager.AddToRoleAsync(manager, UserRole(1));
        ...
        var headchefResult = await _userManager.CreateAsync(headChef, "12345678!Fpt");
        if (!headchefResult.Succeeded)
        {
            throw new AppException("Không thể tạo Head Chef");
        }
        await _userManager.AddToRoleAsync(headChef, UserRole(4));
