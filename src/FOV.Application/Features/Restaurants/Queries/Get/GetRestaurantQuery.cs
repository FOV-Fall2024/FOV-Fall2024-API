...
public record GetRestaurantCommand(Guid? RestaurantId, string? RestaurantName, string? Address, string? RestaurantPhone, string? RestataurantCode) : IRequest<List<GetRestaurantResponse>>;
public record GetRestaurantResponse(Guid RestaurantId, string RestaurantName, string Address, string RestaurantPhone, string RestataurantCode);
public class GetRestaurantQuery(IUnitOfWorks unitOfWorks) : IRequestHandler<GetRestaurantCommand, List<GetRestaurantResponse>>
{
    ...
    public async Task<List<GetRestaurantResponse>> Handle(GetRestaurantCommand request, CancellationToken cancellationToken)
    {
        ...
        var filterEntity = new Restaurant
        {
            ...
            RestataurantCode = string.IsNullOrEmpty(request.RestataurantCode) ? string.Empty : request.RestataurantCode
        };
        var filterRestaurant = restaurants.AsQueryable().CustomFilterV1(filterEntity);
        return filterRestaurant.Select(restaurant => new GetRestaurantResponse(restaurant.Id, restaurant.RestaurantName ?? string.Empty, restaurant.Address ?? string.Empty, restaurant.RestaurantPhone ?? string.Empty, restaurant.RestataurantCode ?? string.Empty)).ToList();
    }
}
