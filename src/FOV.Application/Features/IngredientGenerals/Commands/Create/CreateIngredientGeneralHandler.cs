...
public record CreateIngredientGeneralCommand(string Name, string Description, Guid IngredientType, int IngredientMeasureType) : IRequest<Guid>;
public class CreateIngredientGeneralHandler(IUnitOfWorks unitOfWorks) : IRequestHandler<CreateIngredientGeneralCommand, Guid>
{
    ...
    {
        IngredientGeneral ingredientGeneral = new(request.Name, request.Description, request.IngredientType, TransferType.Transfer(request.IngredientMeasureType));
