...
public record CheckInCommand(Guid WaiterScheduleId, DateTime CheckInTime, double Latitude, double Longitude) : IRequest<Guid>;
public class CheckInHandler(IUnitOfWorks unitOfWorks) : IRequestHandler<CheckInCommand, Guid>
{
    ...
    {
        var waiterSchedule = await _unitOfWorks.WaiterScheduleRepository.GetByIdAsync(request.WaiterScheduleId, ws => ws.Shift, ws => ws.Attendances, ws => ws.User);
        if (waiterSchedule == null) throw new AppException("Không tìm thấy lịch làm việc.");

        var shift = waiterSchedule.Shift ?? throw new AppException("Không tìm thấy thông tin ca làm việc.");
        ...
        if (existingAttendance != null)
            throw new AppException("Nhân viên đã check-in trước đó.");

        if (!waiterSchedule.User.RestaurantId.HasValue)
        {
            throw new AppException("Không tìm thấy thông tin nhà hàng.");
        }
        var restaurant = await _unitOfWorks.RestaurantRepository.GetByIdAsync(waiterSchedule.User.RestaurantId.Value);
        if (restaurant == null) throw new AppException("Không tìm thấy thông tin nhà hàng.");
