...
public sealed record ViewProfileResponse(string UserId, string LastName, string FirstName);
internal class ViewProfileHandler(UserManager<User> userManager, IClaimService claimService) : IRequestHandler<ViewProfileCommand, ViewProfileResponse>
{
    ...
    {
        User response = await _userManager.FindByIdAsync(_claimService.UserId) ?? throw new Exception();
