...
public class GetDetailQuery(IUnitOfWorks unitOfWorks) : IRequestHandler<GetDetailCommand, GetDetailResponse>
{
    ...
    {
        Dish? dish = await _unitOfWorks.DishRepository.GetByIdAsync(request.Id, x => x.DishGeneral.DishGeneralImages, x => x.DishIngredients, x => x.DishGeneral.Category);
        var ingredient = await _unitOfWorks.DishIngredientRepository.WhereAsync(x => x.DishId == request.Id, x => x.Ingredient, x => x.Ingredient.IngredientGeneral.IngredientType);
