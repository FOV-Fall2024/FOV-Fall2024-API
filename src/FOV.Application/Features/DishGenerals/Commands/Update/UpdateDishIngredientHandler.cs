...
public class UpdateDishIngredientHandler(IUnitOfWorks unitOfWorks) : IRequestHandler<UpdateProductGeneralCommand, Result>
{
    ...
    {
        var existingImages = await _unitOfWorks.DishGeneralImageRepository
                                              .WhereAsync(x => x.DishGeneralId == dishGeneralId);

        foreach (var image in existingImages)
        {
            _unitOfWorks.DishGeneralImageRepository.Remove(image);
        }

        var newImages = images.Select((imageUrl, index) =>
                        new DishGeneralImage(imageUrl, dishGeneralId, index + 1))
                               .ToList();
